...
func newCAFromConfig(config *gatewayconfig.SSHCAConfig, vaultClient *VaultClient, logger *zap.Logger) (*caConfig, error) {
	if config == nil {
	...
	if config.Vault != nil {
		return newVaultCA(vaultClient, config.Vault)
	}
...
// newVaultCA creates Vault-backed CAs using a pre-configured VaultClient.
func newVaultCA(vc *VaultClient, vaultConfig *gatewayconfig.SSHCAVaultConfig) (*caConfig, error) {
